...
export const StreamingKeys = {
    ...
    models: {
        ...
        batch: (batchId: string): QueryKey => [...StreamingKeys.root, 'batch', batchId],

        // Returns a pattern for matching all model streams
        all: (): QueryKey => [...StreamingKeys.root, 'model'],
    },

    // Message streaming operations
    messages: {
        // Creates a query key for a specific message stream
        stream: (messageId: string): QueryKey => [...StreamingKeys.root, 'message', messageId],

        // Returns a pattern for matching all message streams
        all: (): QueryKey => [...StreamingKeys.root, 'message'],
    },

    // Thread streaming operations
    threads: {
        // Creates a query key for a specific thread stream
        stream: (threadId: string): QueryKey => [...StreamingKeys.root, 'thread', threadId],

        // Returns a pattern for matching all thread streams
        all: (): QueryKey => [...StreamingKeys.root, 'thread'],
    },
...
export const StreamingKeyMatchers = {
    ...
    isModelStream: (queryKey: QueryKey): boolean =>
        queryKey.length >= 3 && queryKey[0] === StreamingKeys.root[0] && queryKey[1] === 'model',
