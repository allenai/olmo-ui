...
from dolma_search import custom, error, glog
...
def create_app() -> proxy_fix.ProxyFix:
    ...
    app = flask.Flask("dolma_search", static_url_path="/static")
    app.json = custom.JSONProvider(app)
    app.get("/health")(lambda: ("", 204))
    app.register_blueprint(v1.Server(), url_prefix="/v1")
